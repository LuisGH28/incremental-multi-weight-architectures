...
export function renderFlow(elements, ev) {
  ...
  const nInputs = arch.n_inputs || ev.n_inputs || 784;
  const nOutputs = arch.n_outputs || arch.n_out || ev.n_outputs || ev.n_out || 10;
  const fastWeightLines = arch.n_fw || ev.n_fw_lines || 3;
  ...
  if (arch.use_dual) {
    html += svgText(280, 86, `w + fw1 + fw2 + fw3 (${fastWeightLines} fast-weight lines)`, 'text-anchor="middle" class="flow-fw3"');
  }
