...
==========================
HTTP/SSE server for the fw3 MNIST dashboard.
...
def main():
    ap = argparse.ArgumentParser(
        description="SSE dashboard for fw3 MNIST neuroevolution"
    )
    mode_grp = ap.add_argument_group("Data format")
    mode_grp.add_argument("--csv", action="store_true",
                          help="Read CSV instead of binary IDX")
    mode_grp.add_argument("--label-first", action="store_true",
                          help="In CSV input, the first column is the label")

    idx_grp = ap.add_argument_group("Binary IDX paths")
    idx_grp.add_argument("--train-images", default=None,
                         help="Training images file (.ubyte/.gz)")
    idx_grp.add_argument("--train-labels", default=None,
                         help="Training labels file (.ubyte/.gz)")
    idx_grp.add_argument("--test-images",  default=None,
                         help="Test images file (.ubyte/.gz)")
    idx_grp.add_argument("--test-labels",  default=None,
                         help="Test labels file (.ubyte/.gz)")

    csv_grp = ap.add_argument_group("CSV paths")
    csv_grp.add_argument("--train", default=None, help="Training CSV")
    csv_grp.add_argument("--test",  default=None, help="Test CSV")

    ap.add_argument("--data-dir", default="./data",
                    help="Root directory for MNIST files")
    ...
    ap.add_argument("--log",  type=str, default="neuroevo.log",
                    help="Path for the server-side log file")
    args = ap.parse_args()
    ...
    if args.csv:
        cmd.append("--csv")
    if args.label_first:
        cmd.append("--label-first")
    if args.train_images:
        cmd += ["--train-images", args.train_images]
    if args.train_labels:
        cmd += ["--train-labels", args.train_labels]
    if args.test_images:
        cmd += ["--test-images", args.test_images]
    if args.test_labels:
        cmd += ["--test-labels", args.test_labels]
    if args.train:
